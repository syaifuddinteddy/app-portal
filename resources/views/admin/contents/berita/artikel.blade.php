            ...
            <div class="box box-success">
                <div class="box-header with-border">
                    ...
                    <div class="pull-right">
                        <a href="{{URL::route('artikelAdd')}}"><button type="button" class="btn btn-success"><i class="fa fa-plus"></i> Tambah Data Artikel</button></a>
                    </div>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
                    <table id="tbInfo_Pejabat" class="table table-bordered table-striped">
                        ...
                        <tbody>
                        @foreach($berita as $idx => $berita)
                        <tr>
                            <td>{{$berita->judul}}</td>
                            <td>{!! strip_tags(substr($berita->narasi,0,200)) !!}</td>
                            <td>{{$berita->tgl_entri}}</td>
                            <td align="center" class="col-md-1">
                                @if($berita->status == 1)
                                    <span class="badge bg-green"><i class="fa fa-warning"></i> Tampilkan</span>
                                @else
                                    <span class="badge bg-red"><i class="fa fa-warning"></i> Tidak ditampilkan</span>
                                @endif
                            </td>
                            <td align="center">
                                <a class="btn btn-small btn-warning" href="{{ URL::route('artikelEdit', $berita->id_berita)}}"><div class="fa fa-pencil"></div></a>
                                <a class="btn btn-small btn-danger" href="{{ URL::route('artikelDelete', $berita->id_berita) }}"><div class="fa fa-file"></div></a>
                            </td>
                        </tr>
                        @endforeach
                        </tbody>
